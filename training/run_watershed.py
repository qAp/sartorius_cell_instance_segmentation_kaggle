...
def _setup_parser():
    ...
    add('--data_class', type=str, default='Watershed')
    add('--model_class', type=str, default='WatershedNet')
...
def main():
    ...
    model_checkpoint_callback = pl.callbacks.ModelCheckpoint(
        filename=(
            f'fold{args.fold:d}-' +
            'epoch{epoch:03d}-val_loss{val_loss:.3f}-val_ncorrect{val_ncorrect:.3f}'
        ),
        monitor='val_loss',
        mode='min',
