...
Inference.  If the image size is not a whole multiple of 32, it is padded to be.  It's also always padded by 16 on all sides with the symmetric mode.  For the pipeline with a 2-channel target and sigmoid output activation, the model output is of shape (height, width, 2).  This is averaged over a model ensemble and over 8 testtime augmentation predictions. 
